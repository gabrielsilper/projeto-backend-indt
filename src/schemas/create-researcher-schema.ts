import { isAdult } from "utils/date-utils";
import { z } from "zod";
...
export const createResearcherSchema = z.object({
	...
	degree: z
		.number(
			"Degree is required and must be a number. 0 - INDEFINIDO, 1 - GRADUACAO, 2 - ESPECIALIZACAO, 3 - MESTRADO, 4 - DOUTORADO",
		)
		.int("Degree must be an integer")
		.min(
			0,
			"Degree must be at least 0. 0 - INDEFINIDO, 1 - GRADUACAO, 2 - ESPECIALIZACAO, 3 - MESTRADO, 4 - DOUTORADO",
		)
		.max(
			4,
			"Degree must be at most 4. 0 - INDEFINIDO, 1 - GRADUACAO, 2 - ESPECIALIZACAO, 3 - MESTRADO, 4 - DOUTORADO",
		),
	...
	birthDate: z.coerce
		...
		.refine(
			(date) => isAdult(date),
			"Researcher must be at least 18 years old",
		),
});
